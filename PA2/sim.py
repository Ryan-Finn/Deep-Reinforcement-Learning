...
def main():
    ...
    def init_patches():
        y = states[:, 0]
        ps = singlePendulumCart.draw(ax, y)
        for patch in ps:
            ax.add_patch(patch)
        return ps
    ...
    def animate(it):
        y = states[:, it]
        return singlePendulumCart.draw(ax, y)

    a = anim.FuncAnimation(
        fig, animate, init_func=init_patches, interval=(T[1] - T[0]) * 1000, frames=states.shape[1], repeat=False,
        blit=True)
